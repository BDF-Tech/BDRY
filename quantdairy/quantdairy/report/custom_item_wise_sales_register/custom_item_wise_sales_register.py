...
def _execute(filters=None, additional_table_columns=None, additional_conditions=None):
    ...
    # ⭐ ADDED FOR MRP PRICE (OPTIMIZED FETCH)
    item_codes = list(set(d.item_code for d in item_list))

    mrp_price_map = {}
    if item_codes:
        mrp_data = frappe.db.sql(
            """
            SELECT item_code, price_list_rate
            FROM `tabItem Price`
            WHERE price_list = 'MRP Price'
            AND item_code IN (%s)
            """
        ...
        for row in mrp_data:
            mrp_price_map[row.item_code] = row.price_list_rate
    # ⭐ END MRP ADD
    ...
    for d in item_list:
        ...
        # ⭐ ADD: MRP PRICE + MRP VALUE (ADDED FIELD ONLY)
        mrp_price = mrp_price_map.get(d.item_code, 0)
        mrp_value = mrp_price * (d.stock_qty or 0)
...
def get_columns(additional_table_columns, filters):
    ...
    # ⭐ ADD MRP COLUMNS AT THE VERY END (AS REQUESTED)
    columns += [
        {
            "label": _("MRP Price"),
            "fieldname": "mrp_price",
